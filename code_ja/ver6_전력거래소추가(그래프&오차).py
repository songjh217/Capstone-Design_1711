...
test_kpx_data = pd.read_csv('/content/drive/My Drive/Colab Notebooks/캡스톤/data/bidforecastgen_land_2019.csv', usecols=['MAX'])
...
#전력거래소 정리
    ...
    kpx_test_y_numerator = kpx_testY - np.min(kpx_testY, 0) 
    kpx_test_y_denominator = np.max(kpx_testY, 0) - np.min(kpx_testY, 0)
    kpx_test_y_set = kpx_test_y_numerator / (kpx_test_y_denominator + 1e-7)
    kpx_testY_tensor = torch.FloatTensor(kpx_test_y_set)


    #kxp_예측값 4분기
    kxp_part_1 = kpx_testY[:90]
    kxp_part_2 = kpx_testY[90:181]
    kxp_part_3 = kpx_testY[181:273]
    kxp_part_4 = kpx_testY[273:]
    
    #kxp_예측값_정규화
    kxp_part_n_1 = kpx_testY_tensor[:90]
    kxp_part_n_2 = kpx_testY_tensor[90:181]
    kxp_part_n_3 = kpx_testY_tensor[181:273]
    kxp_part_n_4 = kpx_testY_tensor[273:]
    ...
    kpx_MSE_whole = MSE(testY_tensor,kpx_testY_tensor)
    kpx_MSE_1 = MSE(Y_tensor_1,kxp_part_n_1) # 실제:실제값
    kpx_MSE_2 = MSE(Y_tensor_2,kxp_part_n_2)
    kpx_MSE_3 = MSE(Y_tensor_3,kxp_part_n_3)
    kpx_MSE_4 = MSE(Y_tensor_4,kxp_part_n_4)


    
    kpx_RMSE_whole = torch.sqrt(kpx_MSE_whole) #나눠준 후 루트
    kpx_RMSE_1 = torch.sqrt(kpx_MSE_1)
    kpx_RMSE_2 = torch.sqrt(kpx_MSE_2)
    kpx_RMSE_3 = torch.sqrt(kpx_MSE_3)
    kpx_RMSE_4 = torch.sqrt(kpx_MSE_4)



    kpx_MAPE_whole =  np.mean(np.abs((testY - kpx_testY) / testY)) * 100
    kpx_MAPE_1 =  np.mean(np.abs((Y_1 - kxp_part_1) / Y_1)) * 100
    kpx_MAPE_2 =  np.mean(np.abs((Y_2 - kxp_part_2) / Y_2)) * 100
    kpx_MAPE_3 =  np.mean(np.abs((Y_3 - kxp_part_3) / Y_3)) * 100
    kpx_MAPE_4 =  np.mean(np.abs((Y_4 - kxp_part_4) / Y_4)) * 100
...
kpx_RMSE_whole = kpx_RMSE_whole.numpy()
kpx_RMSE_1 = kpx_RMSE_1.numpy()
kpx_RMSE_2 = kpx_RMSE_2.numpy()
kpx_RMSE_3 = kpx_RMSE_3.numpy()
kpx_RMSE_4 = kpx_RMSE_4.numpy()
...
print('kpx')
print()
print('RMSE(전체) : ',kpx_RMSE_whole)
print('MAPE(전체) : ',kpx_MAPE_whole)
print()
print('RMSE(1분기) : ',kpx_RMSE_1)
print('RMSE(2분기) : ',kpx_RMSE_2)
print('RMSE(3분기) : ',kpx_RMSE_3)
print('RMSE(4분기) : ',kpx_RMSE_4)
print()
print('MAPE(1분기) : ',kpx_MAPE_1)
print('MAPE(2분기) : ',kpx_MAPE_2)
print('MAPE(3분기) : ',kpx_MAPE_3)
print('MAPE(4분기) : ',kpx_MAPE_4)
...
plt.legend(['kpx','original', 'prediction'])
plt.show()
