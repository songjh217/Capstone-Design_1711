...
change_test = np.reshape(test_set_np,(10,7,data_parameter_test)) # 70 / 7 = 10으로 딱 나누어 떨어짐.(일주일씩 정렬 //토,일 분리해주려고)

# 화 수 목 금 토 일 월 [4],[5] >> 2020년 6월 23일은 화요일임
weekend_test = change_test[:, 4:6,:] # 토~일 자르기

sat = np.delete(change_test,4,axis=1) # 토요일 자르기
week_test = np.delete(sat,4,axis=1) # 일요일 자르기


WEEK_test = np.reshape(week_test,(50,data_parameter_test)) #사이즈변형 (build_dataset들어가게 하려고)
