...
train_data = pd.read_csv('/content/train_heat(14-18)_1.csv', usecols=['rainfall_all','max_power'])
trainxy_data = train_data.values
train_set = trainxy_data [::-1] 

test_data = pd.read_csv('/content/test_heat(19).csv', usecols=['rainfall_all', 'max_power'])
...
df1 = pd.DataFrame([[RMSE_whole, RMSE_sum, RMSE_1, RMSE_2, RMSE_3, RMSE_4],[MAPE_whole, MAPE_sum, MAPE_1,MAPE_2,MAPE_3,MAPE_4]]) 
df2 = pd.DataFrame(prediction_powerV_nn) 
df1.to_excel('0525data1.xlsx', index=False)
df2.to_excel('0525data2.xlsx', index=False)
